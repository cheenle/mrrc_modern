...
**Diagrams are artifacts.** `SDD/diagrams/*.svg` (and `docs/images/ui-guide-*.svg`) are the single
source; `website/sdd/diagrams/` and `website/images/` are generated by `build_sdd.py` /
`build_guide.py` — never edit a copy. Each diagram carries a `diagram-version` marker (allowed to lag
at most two releases) plus the keywords of the capability it draws, so a feature change fails the
check until the picture is updated. Redraw on capability changes, then rebuild the site.
