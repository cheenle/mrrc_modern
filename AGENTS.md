...
| `recorder.py` | Server-side QSO recording (AD-017): `RecordingSession` = 16 kHz mono timeline (monotonic timestamps, 50 ms jitter tolerance, silence gap fill, incremental lameenc MP3 with per-block `flush()`), `_StreamingDecimator` (96-tap FIR 48k→16k), mrrc-compatible naming `<freq>kHz_<date>_<time>.mp3` and `recordings.json` index helpers. `server.py` owns the single writer, which is one-shot per session by design — `_ensure_rec_writer()` recreates it for every session (start / block / stop), because a boot-only writer silently dropped every block of every later recording (2026-09-12 field report: correctly-sized, 100 % silent MP3s); a full queue is the only drop path, and a stop is queued, never run on the event loop **16 kHz is a storage domain, not the codec/device domain** — 44.1k→48k goes through `audio_resample`, nothing here re-enters the audio path |
| `cq_player.py` | One-touch CQ call (AD-020): WAV parse → 48 kHz mono normalise → 20 ms frames, fed to the existing TX queue only while the device queue is shallow (`CQ_LOW_WATER_FRAMES`, so a call never manufactures `queue_drops`); one call per press, abortable; key/unkey/broadcast callbacks are injected by `server.py` so the module stays hardware-free. `server.py` checks the TX gate before the asset, drops browser mic frames while calling (`_tx_cq_mic_drops`), refuses PTT/TUNE, aborts when the initiating client or the last client disconnects, and warns at startup when the asset is unusable |
| `support_bundle.py` | Support diagnostics bundle (AD-021): key-allow-listed config redaction + secret-value pass, bounded/log-aligned log tails with a 512 KB/128 KB degradation ladder, auto-triage `summary.txt` matching this repo's real log lines, version detection (`version.txt` → `.iss` → CHANGELOG) and zip assembly with per-file SHA-256, newest-5 retention. Stdlib only and no app imports, so it stays unit-testable and hot-fixable |
| `launcher_log.py` | Startup tee: drains the child's stdout/stderr into `logs/server-stdout.log` during the startup window (the only witness of a server that dies before its own logging exists), then keeps draining without writing so a chatty server can never block on a full 64 KB pipe; echoes to the console as before |
| `upgrade_core.py` / `dev_tools/make_latest_json.py` | Update channel, slice 1 (AD-022): manifest fetch/validate/compare, the `state.json` schema that makes an upgrade **provable** (only the new version's boot may write `ok`), and the manifest generator whose size/SHA-256 come from the artifacts themselves. Read-only: `GET /api/update/check`. Slice 2 (download → verify → PTT gate → installer → self-proof) is deliberately not started |
| `support_answers.py` / `dev_tools/support_autopilot.py` | Support chain 2/4 (AD-021 amendment): the autopilot polls the receiver, unpacks a bundle, builds the digest and has `pi` (read-only tools, repo as cwd) answer through a fixed JSON contract; `support_answers.py` holds the contract, the card/page renderer and the **public-page privacy filter**. Drafts by default, `--publish` updates the answers page + commits + rsyncs that one file, `need_more_info` never publishes; idempotent state, ≤2 bundles per run, 540 s pi timeout |
| `support_board.py` / `dev_tools/fde_board.py` | FDE loop board (support chain 4/4): every finished analysis lands classified on a public kanban (<www.vlsc.net/mrrc_modern/board/>) — **classification is the decision**: `noise` closes as 已答复 (answer published, no code work), `bug` auto-schedules into the unattended implementation queue, `feature` waits in 待决策 for the operator (`fde_board.py decide`); an un-answered report (`need_more_info`) never closes. `autopilot --implement` then runs unattended implementation — `pi` proposes a unified-diff **JSON contract**, guarded (protected paths: tooling-guard JS files, `certs/`, `packaging/`; ≤8 files), applied only to an isolated `fde/<id>` branch and committed only when the full unittest suite is green; `main` is never touched unattended. Same redaction filter as the answers page |
| `tools/support_receiver/` | Vendored stdlib receiver for this product's own instance (port 8098, `/var/www/support-modern`, `product` in meta.json); `deploy_support_receiver.sh` deploys the unit + nginx path idempotently |
| `deploy_listen_proxy.sh` | Public listen-only URL (V2.59): idempotently inserts the `/mrrc_modern/listen` block (page + login + listen.js + worklet + modules + api + `/WS*` with upgrade headers) into www.vlsc.net's nginx and reloads. Backend is the radio server **by DNS name** (`LISTEN_BACKEND=https://radio.vlsc.net:8888`, AAAA over IPv6, `proxy_ssl_verify off`, no SSH tunnel); `proxy_pass` uses a variable + per-location `resolver … valid=300s` so a changed home IPv6 re-resolves without a reload. Prefix locations must be `^~` — the site's regex `\.js$` location otherwise serves local 404s. Hostname change = edit + re-run (stale blocks are removed first) |
...
| `backends/yaesu/` | **Shared Yaesu ASCII-CAT core (FTDX10, FTDX101D, FTDX101MP, FTX-1F)** — `yaesu_profiles.py` (`YaesuModelProfile`/`MeterCal` per model: mode registers **and** the separate CAT-character lookup — the FTX-1's `H`/`I` C4FM codes are not hex digits; filter slot tables; bands; attenuator/preamp steps; `power_format` (`PC1`/`PC2`/`auto`, FTX-1 detects its Field-head vs SPA-1 class); per-model S-meter curves taken from Hamlib `newcat.c:339` / `ftdx101.h:164` / `ftx1/ftx1.h:105`; `provenance` per table; `verified=False`), `cat_core.py` (`YaesuCatController`: the FT-710's field-proven transport — `;` framing, AI-frame prefix filtering, write-only sets, priority preemption for PTT/TUNE, ENXIO vs transient classification, reconnect — parameterised by profile, plus the FTX-1 `VM000;` leave-memory step from Hamlib `ftx1.c:881`), `backend.py` (`YaesuBackend` derives capabilities/tables/poll items from `_profile`; abstract surface satisfied by delegates bound at import time with the frozen abstract set cleared; **TX gate** refuses keying on unverified models unless `MRRC_ALLOW_UNVERIFIED_TX=1`; read-only `ID;` identity check logs the observed ID and only warns on mismatch; `scope_type="none"` → the existing S-meter synthesiser covers the UI). `backends/ft710/` is deliberately **not** migrated (phases 2-4 in the spec) |

Frontend assets in `static/`:

- `index.html` — SPA shell (mobile-first responsive layout)
- `listen.html` + `listen.js` — listen-only interface (`/listen`, V2.59): frequency/mode tuning, memory recall, S-meter, waterfall, RX audio + browser volume. Self-contained (reuses `rx_worklet_processor.js` + WASM Opus, never loads the guarded monoliths); the role is enforced server-side, the page merely offers nothing else. **Prefix-aware** (`URL_BASE` from `location.pathname`): same files serve direct `/listen` and the public reverse proxy `https://www.vlsc.net/mrrc_modern/listen`
...
- `modules/atr1000.js` — ATR1000 tuner WS client
- `audio/cq.wav` — packaged CQ recording played by the one-touch CQ key (48 kHz mono, ~6.1 s; replaced via `MRRC_LOG_DIR` (server log + bundle output dir; the desktop launchers point it at the user data directory), `MRRC_SUPPORT_URL` (receiving end for uploaded bundles), `MRRC_CQ_FILE`) + meter row + ATR TUNE button (inert unless `atr1000Enabled`)
...
Environment variables: `MRRC_RADIO_MODEL` (backend key: `ft710`/`ic7300`/`ic7300mk2`/`ic705`/`ic7610`/`ic7760`, default `ft710`), `MRRC_RECORDINGS_DIR` (default `<runtime dir>/recordings`; launchers point it at the user data dir), `MRRC_RECORDINGS_BITRATE` (64), `MRRC_RECORDINGS_MAX_SESSION_MIN` (240, 0 = unlimited — stops a forgotten recording, never deletes), `MRRC_CQ_FILE` (CQ recording for the one-touch key, default `static/audio/cq.wav`; any 16-bit WAV, 30 s max), `MRRC_ALLOW_UNVERIFIED_TX` (opt-in transmit enable for the three hardware-unverified Icom models; default off), `MRRC_LISTEN_PASSWORD` (listen-only login for `/listen` — freq/mode + RX audio/spectrum only, no TX or device settings; empty = disabled), `IC7300_CIV_ADDR` (IC-7300 address, default `0x94`), `IC7300MK2_CIV_ADDR` (MK2 address, default `0xB6`), `MRRC_SERIAL_PORT`, `MRRC_BAUD_RATE` (backend default: 38400/115200), `MRRC_WEB_PORT`, `MRRC_WEB_PASSWORD`, `MRRC_WEB_HOST`, `MRRC_AUDIO_RX_DEVICE`, `MRRC_AUDIO_TX_DEVICE`, `MRRC_FTDI_LIB_DIR`, `MRRC_FT4222_CLK_DIV`, `MRRC_SCOPE_PORT`, `MRRC_SCOPE_BAUD`, `MRRC_ATR1000_HOST`, `MRRC_ATR1000_PORT`. All applicable variables also honor legacy `FT710_*` aliases through the config fallback (`MRRC_*` wins).
