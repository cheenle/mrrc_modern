...
| `deploy_listen_proxy.sh` | Public listen-only URL (V2.59): idempotently inserts the `/mrrc_modern/listen` block (page + login + listen.js + worklet + modules + api + `/WS*` with upgrade headers) into www.vlsc.net's nginx and reloads. Backend is the radio server's **public IPv6** (`LISTEN_BACKEND`, HTTPS :8888, `proxy_ssl_verify off`, no SSH tunnel); prefix locations must be `^~` — the site's regex `\.js$` location otherwise serves local 404s. Address change = edit + re-run (stale blocks are removed first) |
