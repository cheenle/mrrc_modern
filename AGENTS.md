...
- `audio/cq.wav` — packaged CQ recording played by the one-touch CQ key (48 kHz mono, ~6.1 s; replaced via `MRRC_CQ_FILE`) + meter row + ATR TUNE button (inert unless `atr1000Enabled`)
...
Run the full suite with `python -m unittest discover -s tests -v` (currently 1055 tests across 53 modules). At minimum: `python -m py_compile *.py`. Hardware-dependent changes should document: connecte
