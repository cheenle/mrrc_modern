...
### Fixed — CAT 掉线后恢复要 62 秒

- **重连不再做 24 条全量状态同步。** 现场测得电台 USB 串口桥重枚举后，watchdog 的
  `initial_state_sync` 会占住串口锁 **约 62 秒**（其间用户命令排队、日志空白）。这些字段
  全部由轮询分层在数秒内覆盖（0.1 s/0.5 s/2 s/5 s），因此重连只恢复连接标志并重跑频谱
  初始化；未读到的字段也不会再被默认值清零。
- **日志静音**：重连尝试首次 INFO、后续 DEBUG，掉线超 15 秒只发一条带硬件提示的 WARNING；
  `connect()` 失败仅首次打印端口列表；ENXIO（"Device not configured"）会明确提示是 USB
  桥物理层消失（检查线缆/供电/勿用无源 HUB）。
