...
| YaesuModelProfile | model_key, display_name, mode_numbers, mode_codes, filter_widths, bands, att_steps, power_format, s_meter_cal, verified, provenance | Per-model Yaesu facts (spec 2026-09-12 §5): the mode **register** (int, the `RadioState` contract) and the separate CAT **character** (H/I are not hex digits), filter slots, bands, attenuator/preamp steps, power class, S-meter curve and the offline source of every table |
