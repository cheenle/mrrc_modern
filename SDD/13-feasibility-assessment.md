...
| I8 | `serve_static` path traversal — `STATIC_DIR / path` (server.py) builds the response path from the request URL without `resolve()` + containment check, so an authenticated non-browser client can read arbitrary server-readable files (e.g. `GET /../server.py`, cert keys). Browsers normalize `..`, so exposure is raw-HTTP clients. | High | Open | Resolve the joined path and reject unless `is_relative_to(STATIC_DIR)` before `FileResponse`; add a regression test |
| I9 | Login password compared with `!=` (non-constant-time, timing side channel) and a weak default password only logs a warning — a fresh install that skips the warning is effectively open. | Medium | Open | Switch to `hmac.compare_digest`; escalate weak-password handling (first-login forced change or startup banner); consider shrinking the 30-day auth cookie |
| I10 | Web client audio/spectrum subchannels (`/WSaudioRX`, `/WSaudioTX`, `/WSspectrum`) have no independent reconnect — only `/WSradio` auto-reconnects, so a transient drop yields "controls alive but audio dead" until the control channel also cycles. | Medium | Open | Give each subchannel its own exponential-backoff reconnect (pattern exists in `static/modules/atr1000.js`) |
| I11 | iOS app PTT release race (P0 in docs/IOS_APP_ANALYSIS.md): `DragGesture.onEnded` sends `ptt:false` only when the server echo already shows `tx_status > 0`, and there is no client watchdog/scenePhase guard — fast taps over WAN can leave the radio keyed up. Approved PTTManager fix design not yet implemented (0/N tasks). | High | Open | Implement unconditional release on gesture end + 500 ms×3 watchdog + scenePhase force-release (mirror FT710Android `PTTManager`) |
