...
| Status | **v1.14.2 dual-platform release + same-day S-meter flicker hotfix (SDD V2.39 — scope "no data" sentinel -1: the IC-7300's CI-V scope segments carry no S-meter byte, and the old 0-initialized field forced radio.s_meter to 0 on every waveform between CAT polls, flickering the UI S-meter)**: macOS DMG 55,898,038 bytes (SHA-256 `88ed7c626e86b8cb2ba03a5ce2419c9313fece0e97eb579da75b8dda6c6bbb75`) + Windows Setup 45,435,022 bytes (SHA-256 `a7ee16674c0db29a80fbb072301581a3ecf4805e6705382db380ea534862c706`, built on the Win11 KVM with 724 tests / 3 PyInstaller targets / Inno Setup / bundled-file inspection / cross-host SHA-256 all green); carries the V2.37 IC-7300 model↔baud linkage + audio duplicate recovery fixes (connection-dialog save and first-run probing align MRRC_BAUD_RATE with the selected model — the legacy template pre-filled 38400, which starved the CI-V scope stream, field log 2026-09-10 — and -9999 audio-open retries now resolve a different host-API duplicate). Raspberry Pi rpi64 image packaging line added (SDD V2.36); both installers also carry the V2.31/V2.32 RX recording-quality fixes (capture reopened after every TX→RX transition on all platforms; RX broadcast no longer frame-trims catch-up bursts); IC-7300/MK2 physical hardware acceptance pending; FT710Android device acceptance pending; I8/I10/I12 resolved, I9/I11 partially resolved (remaining: forced first-login change; iOS watchdog/scenePhase); open issues tracked in §13.4 |
