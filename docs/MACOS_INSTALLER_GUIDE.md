...
## 首次启动：允许「麦克风」（这是接收音频，不是录音）

应用是电台控制与**转发放大器**，需要读取电台 USB 声卡的**音频输入**才能把收到的声音传到浏览器。
macOS 把音频输入归入**麦克风**权限类，所以首次启动会弹一次询问 —— 请点**允许**
（也可事后在 系统设置 → 隐私与安全性 → 麦克风 中打开；应用名为 **MRRC Modern**）。

若拒绝授权，症状很隐蔽：**软件一切正常、日志说 RX 捕获流已启动，但完全没有声音**，
CoreAudio 不会报错（它允许流打开、只把数据填零）。此时应用日志会提示去系统设置里允许麦克风。

> **v1.18.0 及更早的 macOS 包请勿使用**：那些包的应用签名实际从未成功，Gatekeeper 会报
> 「已损坏，无法打开」（该提示右键也绕不过去），且缺少上述权限说明键（连询问都不会弹）。
> 请下载 v1.18.1 或更新版本。ad-hoc 签名下权限授权绑定当次构建，因此**每次升级都会重新询问一次**。

## 一、安装（约 1 分钟）

1. 打开下载的 `MRRC-Modern-v1.18.1-arm64.dmg`。
