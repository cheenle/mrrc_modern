...
> 适用于 FT-710 Web Control（v1.8.x，2026-08 之后的界面）。本文档逐项说明界面上每一个按钮、滑杆、下拉框的功能与用法，编号与文末插图（图 1 主界面、图 2 菜单）中的琥珀色序号一一对应。
...
| 13 | SWR / Id / Vd 表 | 驻波比（SWR）、末级电流（A）、供电电压（V） | 只读；Id/Vd 仅在 TX 时有效，TX→RX 会清零 |
...
| # | 控件 | 功能 | 操作 |
|---|------|------|------|
| 15 | 模式按钮（如 `USB`） | 按顺序循环切换模式：LSB → USB → CW-U → AM → FM → RTTY-L → DATA-L | 单击循环；需要精确选择时用菜单 → Mode Selection（序号 40） |
| 16 | 波段按钮（如 `20m`） | 循环切换业余波段：160m/80m/60m/40m/30m/20m/17m/15m/12m/10m/6m/4m；切换后频率跳到该波段默认频率并自动选 VFO-A | 单击循环；菜单 → Band Selection（序号 39）可精确选 |
| 17 | 滤波按钮（如 `2.4k`） | 循环切换当前模式的滤波宽度（SSB：9/13/17/20/23 档；其他模式各有对应表）。服务端设置后读回电台实际索引并回显 | 单击循环 |
| 18 | ATT 衰减 | 射频衰减器 4 档循环：0 → 1 → 2 → 3（对应电台 `RA` 命令 0–3） | 单击循环 |
| 19 | PRE 前置放大 | 前置放大器 3 档循环：0（关）→ 1 → 2（对应 `PA` 命令） | 单击循环 |
...
| 22 | AN | 自动陷波（Auto Notch）开关 | 单击开/关 |
...
3. 发射期间**频谱暂停**（电台会干扰频谱数据流），瀑布图显示「TX 发射中 — 频谱暂停」是正常现象。
