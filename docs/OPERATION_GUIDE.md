...
**整段录音全哑（能播但没声音）**：若日志里 `Recording started` 之后**立刻**出现「正在丢弃」、丢弃数一路涨，最后以 `Recording queue full on stop` 收尾 —— 或录音面板每次都显示 `⚠ 丢失 N 块`、文件明显是静音 —— 那是 v1.15.0 的录音写入端生命周期缺陷（同一进程里从第二次录音起没有消费者）。升级到含该修复的版本并**重启服务**即可；判定办法：`REC` → 停止 → 从菜单「录音」播放，听第一秒有没有声音。
