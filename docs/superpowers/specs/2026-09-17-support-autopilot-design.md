...
| D4 | Page delivery | **Single-file rsync** to `/var/www/vlsc.net/mrrc_modern/answers/index.html` (non-interactive; the full site deploy is interactive and must not run from cron). A failed rsync only warns — the local commit is the source of truth |
