...
[ -f "01-deploy-mrrc/files/opt/mrrc_modern/requirements.txt" ] || {
	echo "FATAL: code tree missing at $PWD"
	exit 1
}
cp -a 01-deploy-mrrc/files/. "$ROOTFS_DIR/"
[ -f "$ROOTFS_DIR/opt/mrrc_modern/requirements.txt" ] || {
	echo "FATAL: code copy failed"
	exit 1
}
[ -f "$ROOTFS_DIR/usr/local/bin/mrrc-show-password" ] || {
	echo "FATAL: helper copy failed"
	exit 1
}
